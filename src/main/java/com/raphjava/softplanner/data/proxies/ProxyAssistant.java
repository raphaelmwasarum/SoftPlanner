...
public class ProxyAssistant
{
	...
	protected <T> T get(Class<T> cl, int id, boolean withRelatives, Consumer<EagerLoader<T>> eagerLoaderAction)
	{
		...
		dataService.read((r) -> 
		{
			...
			if(withRelatives)
			{
				v = v.withRelationships();
			}
			
			if(eagerLoaderAction != null)
			{
				v.eagerLoad(eagerLoaderAction).onSuccess(en -> rez[0] = en).onFailure(() -> System.out.println("proxy data read failure."));
			}
